...
st.subheader("Conclusão para o Trabalho:")
st.info(f"Observou-se uma correlação positiva fraca entre o número de votos e a avaliação média das séries. O coeficiente de determinação (R² = {r_quadrado:.4f}) indica que cerca de {r_quadrado*100:.2f}% da variação das notas pode ser explicada pela popularidade. Apesar da força da relação ser baixa, o p-valor pequeno ({p_valor_reg:.2e}) indica forte significância estatística.")



#predição
st.divider()
st.subheader("Modelo Preditivo (Votos + Gênero)")
...
if len(votos_gen) > 1:
    #validaçao da confiabilidade
    r_teste_confiabilidade = calcular_correlacao_pearson_xy(votos_gen, notas_gen)
    
    if abs(r_teste_confiabilidade) < 0.6:
        st.warning("Não é aconselhável a utilização da reta para fazer projeções quando o coeficiente r é muito baixo, pois as estimativas perdem confiabilidade. O resultado abaixo está sendo exibido apenas para demonstração.")
    ...
    st.success(f"**previsão processada**")
